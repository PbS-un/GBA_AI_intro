      ...
      <div className="section-shell py-12 sm:py-14">
        <div className="grid gap-10 border-b border-white/10 pb-10 md:grid-cols-[1fr_auto] md:items-start">
          <div className="flex items-start gap-4">
            <BrandMark compact />
            <div>
              <p className="text-xl font-black tracking-[0.03em]">{projectMeta.projectName}</p>
              <p className="mt-1 text-xs font-bold uppercase tracking-[0.12em] text-white/65">{projectMeta.teamName} · 澳門 AI 語音助老守護系統</p>
              <p className="mt-5 max-w-lg text-base font-semibold leading-7 text-teal-100">{projectMeta.slogan}</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-x-6 gap-y-3 md:max-w-md md:justify-end">
            {footerLinks.map((link) => (
              <a key={link.href} href={link.href} className="inline-flex min-h-11 items-center text-sm font-semibold text-white/65 transition-colors hover:text-white">{link.label}</a>
            ))}
            <button type="button" className="flex min-h-11 items-center gap-1.5 text-sm font-semibold text-white/65 transition-colors hover:text-white" onClick={onContact}><Mail size={15} /> 聯絡合作</button>
          </div>
        </div>

        <div className="flex flex-col gap-5 pt-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-4xl">
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-white/65">Important notice · 重要聲明</p>
            ...
            <p className="mt-5 text-xs text-white/60">© 2026 {projectMeta.teamName} · 銀髮一句通 SilverCare Voice. 概念展示網站。</p>
          </div>
