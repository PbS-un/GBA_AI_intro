      ...
      {open && (
        ...
        >
          ...
          >
            ...
            {submitted ? (
              <div className="flex min-h-[430px] flex-col items-center justify-center px-7 py-12 text-center" aria-live="polite">
              ...
              <form className="max-h-[calc(100vh-190px)] overflow-y-auto px-6 py-6 sm:px-8" noValidate onSubmit={handleSubmit}>
                <div className="grid gap-5 sm:grid-cols-2">
                  <Field label="姓名" name="name" required error={errors.name}>
                    <input id="name" name="name" autoComplete="name" value={values.name} onChange={(event) => updateField("name", event.target.value)} aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? "name-error" : undefined} />
                  </Field>
                  <Field label="機構" name="organization" required error={errors.organization}>
                    <input id="organization" name="organization" autoComplete="organization" value={values.organization} onChange={(event) => updateField("organization", event.target.value)} aria-invalid={Boolean(errors.organization)} aria-describedby={errors.organization ? "organization-error" : undefined} />
                  </Field>
                  <Field label="電郵" name="email" required error={errors.email}>
                    <input id="email" name="email" type="email" autoComplete="email" value={values.email} onChange={(event) => updateField("email", event.target.value)} aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? "email-error" : undefined} />
                  </Field>
                  ...
                  <div className="sm:col-span-2">
                    <Field label="合作方向" name="direction" required error={errors.direction}>
                      <select id="direction" name="direction" value={values.direction} onChange={(event) => updateField("direction", event.target.value)} aria-invalid={Boolean(errors.direction)} aria-describedby={errors.direction ? "direction-error" : undefined}>
